...
class UiChatMessage extends StatelessWidget {
  const UiChatMessage({required this.item, super.key, this.date});
  ...
  Widget build(BuildContext context) {
    Color bgColor = context.colors.bg.secondary;
    Color textColor = context.colors.text.main;

    if (item.isMe) {
      bgColor = context.colors.bg.tertiary;
    }

    if (item.isAssistant) {
      bgColor = context.colors.content.success;
      textColor = context.colors.text.constantWhite;
    }
          ...
          children: [
            if (item.isAssistant)
              Padding(
                ...
                child:
                    UiImage(AssetsNew.images.other.jojoAvatar.path, width: 40),
              ),
            if (item.isAssistant)
              Expanded(
                child: UiChatMessageItem(
                  message: Text(
